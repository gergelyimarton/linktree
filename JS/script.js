...
// Interactive gradient background (disabled on phones for performance)
if (!window.matchMedia('(max-width: 550px)').matches) {
    ...
    function animateColors() {
        phase += 0.01;
        phase2 += 0.017; // Different speed for unpredictability
        
        // Oscillate between orange and purple with two axes
        const mix = (Math.sin(phase) + 1) / 2; // 0 to 1
        const mix2 = (Math.cos(phase2) + 1) / 2; // Secondary rotation axis
        ...
        // Interpolate between orange and purple with dual-axis rotation
        const lerp = (a, b, t) => a.map((v, i) => Math.round(v + (b[i] - v) * t));
        
        const [r1, g1, b1] = lerp(orange1, purple1, 0.5 + mix * mix2 * 0.5);
        const [r2, g2, b2] = lerp(orangeDark, purpleDark, 0.4 + mix * 0.6);
        const [r3, g3, b3] = lerp(orange2, purple2, 0.5 + mix2 * 0.5);
        const [r4, g4, b4] = lerp(purpleDark, orangeDark, (mix + mix2) / 2);
        const [r5, g5, b5] = lerp(orange3, purple3, 0.4 + mix * 0.6);
        const [r6, g6, b6] = lerp(orangeDark, purpleDark, 0.6 + (1 - mix2) * 0.4);
        const [r7, g7, b7] = lerp(orange2, purple1, 0.5 + mix2 * mix * 0.5);
        const [r8, g8, b8] = lerp(purpleDark, orangeDark, 1 - mix);
        const [r9, g9, b9] = lerp(purple3, orange3, (1 - mix) * mix2);
        const [r10, g10, b10] = lerp(purpleDark, orangeDark, mix);
        const [r11, g11, b11] = lerp(orange1, purple2, 0.6 + (1 - mix2) * 0.4);
        const [r12, g12, b12] = lerp(purpleDark, orangeDark, (mix + (1 - mix2)) / 2);
